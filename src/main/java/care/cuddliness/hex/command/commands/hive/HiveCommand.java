...
public class HiveCommand extends BaseCommand {
    ...
    private static final HashMap<UUID, UUID> consentInvite = new HashMap<>();
    ...
    public static void onHelp(Player player, String[] args) {
        ...
        MessageUtil.sendMessage("<color:#967CAF>/drone emergencyrelease [droneID/Player] <color:gray>-</color> <color:#ff66ff>Releases a drone from it's latex skin", player);
        ...
        MessageUtil.sendMessage("<color:#967CAF>/hive consent remove [droneID/Player] <color:gray>-</color> <color:#ff66ff>Removes a registered consented drone", player);
        MessageUtil.sendMessage("<color:#967CAF>/hive consent list <color:gray>-</color> <color:#ff66ff>Shows list of all consented drones", player);
        MessageUtil.sendMessage("<reset>                                                                                      ", player);
    }
        ...
        if (args.length == 1) {
            ...
            if (isInteger(args[0]) && drone != null) {
                if (!drone.isActive()) {
                    ...
                    player.kickPlayer(org.bukkit.ChatColor.RED + "You have been registered, rejoin the server for the full effect, drone");
                    return;
                } else {
            ...
            player.getPlayer().setDisplayName("HexDrone" + args[0]);
            NameUtil.changePlayerName(player, "HexDrone" + args[0]);
            Scoreboard scoreboard = Bukkit.getScoreboardManager().getMainScoreboard();
            Team team = scoreboard.registerNewTeam(player.getName());
            team.setPrefix(ChatColor.translateAlternateColorCodes('&', "&8&l⬡ &5" + args[0] + " &7| &r"));
    ...
    public static void onHiveConsentInvite(Player player, String[] args) {
        ...
        Drone drone = HexCore.getHexCore().getDroneDataController().getDrone(player.getUniqueId().toString());
        if (HexCore.getHexCore().getConsentDataController().isConsented(drone, Bukkit.getPlayer(args[0]).getUniqueId().toString())) {
    ...
    public static void onHiveConsentRemove(Player player, String[] args) {
        if (Bukkit.getPlayer(args[0]) == null) {
            ...
            Drone drone = HexCore.getHexCore().getDroneDataController().getDrone(player.getUniqueId().toString());
            if (!core.getConsentDataController().isConsented(drone, Bukkit.getOfflinePlayer(args[0]).getUniqueId().toString())) {
